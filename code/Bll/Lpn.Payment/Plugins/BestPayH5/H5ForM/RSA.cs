...
namespace Kulv.YCF.Payment.Wing.H5ForM
{
    ...
    public sealed class RSA
    {
        ...
        public static string SignByPublicKey(string content, string publicKey, string input_charset)
        {
            ...
            SHA1 sh = new SHA1CryptoServiceProvider();
            byte[] signData = rsaPub.SignData(Data, sh);

            return Convert.ToBase64String(signData);
        }

        public static string SignData(String xiString,string publicKeyI)
        {
            AsnKeyParser keyParser = new AsnKeyParser(publicKeyI);
            RSAParameters publicKey = keyParser.ParseRSAPublicKey();
            RSACryptoServiceProvider DSA = new RSACryptoServiceProvider();
            DSA.ImportParameters(publicKey);
            var array = DSA.Encrypt(Encoding.GetEncoding("utf-8").GetBytes(xiString), false);
