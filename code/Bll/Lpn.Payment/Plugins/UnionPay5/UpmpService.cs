...
namespace Com.UnionPay.Upmp
{
    ...
    public class UpmpService
    {
        ...
        public static bool Trade(Dictionary<String, String> req, Dictionary<String, String> resp)
        {
            ...
            String respString = Com.UnionPay.Upmp.UpmpCore.Post(Com.UnionPay.Upmp.UpmpConfig.GetInstance().TRADE_URL, nvp);
        ...
        {
            String nvp = BuildReq(req, resp);
            String respString = Com.UnionPay.Upmp.UpmpCore.Post(Com.UnionPay.Upmp.UpmpConfig.GetInstance().QUERY_URL, nvp);
            return VerifyResponse(respString, resp);
        }
        ...
        public static String BuildReserved(Dictionary<String, String> req)
        {
            ...
            merReserved.Append("{");
            merReserved.Append(Com.UnionPay.Upmp.UpmpCore.CreateLinkString(req, false, true));  //TODO:?
            merReserved.Append("}");
        ...
        {
            // 生成签名结果
            String signature = Com.UnionPay.Upmp.UpmpCore.BuildSignature(req);

            // 签名结果与签名方式加入请求提交参数组中
            req[Com.UnionPay.Upmp.UpmpConfig.GetInstance().SIGNATURE] = signature;
            req[Com.UnionPay.Upmp.UpmpConfig.GetInstance().SIGN_METHOD] = Com.UnionPay.Upmp.UpmpConfig.GetInstance().SIGN_TYPE;

            return Com.UnionPay.Upmp.UpmpCore.CreateLinkString(req, false, true);
        }
        ...
        public static bool VerifySignature(Dictionary<String, String> para)
        {
            try
            {
                String signature = Com.UnionPay.Upmp.UpmpCore.BuildSignature(para);
                //String respSignature = para[UpmpConfig.GetInstance().SIGNATURE];

                String respSignature = "";

                if (Com.UnionPay.Upmp.UpmpConfig.GetInstance() != null && para.ContainsKey(Com.UnionPay.Upmp.UpmpConfig.GetInstance().SIGNATURE))
                {
                    respSignature = para[Com.UnionPay.Upmp.UpmpConfig.GetInstance().SIGNATURE];
                }
        ...
        private static bool VerifyResponse(String respString, Dictionary<String, String> resp)
        {
            if (respString != null && !"".Equals(respString))
            {
                ...
                try
                {
                    para = Com.UnionPay.Upmp.UpmpCore.ParseQString(respString);
                }
                catch (Exception e)
                {
                    e.ToString();
