    ...
    public class UpmpCore
    {
        ...
        public static Dictionary<String, String> ParaFilter(Dictionary<String, String> para)
        {
            ...
            foreach (String key in para.Keys)
            {
                ...
                if (value == null || value.Equals("") || String.Equals(key, Com.UnionPay.Upmp.UpmpConfig.GetInstance().SIGNATURE, StringComparison.CurrentCultureIgnoreCase)
                    || String.Equals(key, Com.UnionPay.Upmp.UpmpConfig.GetInstance().SIGN_METHOD, StringComparison.CurrentCultureIgnoreCase))
                {
                    continue;
                }
                result[key] = value;
        ...
        public static String BuildSignature(Dictionary<String, String> req)
        {
            ...
            prestr = prestr + QSTRING_SPLIT + MD5(Com.UnionPay.Upmp.UpmpConfig.GetInstance().SECURITY_KEY);
            return MD5(prestr);
        }
        ...
        public static String CreateLinkString(Dictionary<String, String> para, bool sort, bool encode)
        {
            ...
            foreach (String key in list)
            {
                ...
                if (encode && value != null)
                {
                    try
                    {
                        value = HttpUtility.UrlEncode(value, Encoding.GetEncoding(Com.UnionPay.Upmp.UpmpConfig.GetInstance().CHARSET));
                    }
                    catch (Exception ex)
                    {
                        //LogError(ex);
        ...
        private static void putKeyValueToDictionary(StringBuilder temp, bool isKey, String key, Dictionary<String, String> Dictionary)
        {
            ...
            else
            {
                ...
                Dictionary[key] = HttpUtility.UrlDecode(temp.ToString(), Encoding.GetEncoding(Com.UnionPay.Upmp.UpmpConfig.GetInstance().CHARSET));
            }
        }
        ...
        {
            byte[] MD5Source = Encoding.GetEncoding(Com.UnionPay.Upmp.UpmpConfig.GetInstance().CHARSET).GetBytes(str);
