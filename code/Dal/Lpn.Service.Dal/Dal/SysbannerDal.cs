...
namespace Lpn.Service.Dal.Dal
{
    ...
    public partial class SysbannerDal : DalBase
    {
        #region SQL
        //获取整个表数据
        protected const string SqlGetAll = "select * from sysbanner;";
        //新增插入语句
        protected const string SqlInsert = "insert into sysbanner(`Img`,`Desc`,`TimeOutType`,`TimeOutDesc`,`SendType`,`SendDesc`,`TransferType`,`Transfer`,`OperateTime`,`Operater`) values(?Img,?Desc,?TimeOutType,?TimeOutDesc,?SendType,?SendDesc,?TransferType,?Transfer,?OperateTime,?Operater);";
        //获取根据主键查询
        protected const string SqlGetByPriKey = "select * from sysbanner where `ID`=?ID;";
        //根据主键更新整行数据
        protected const string SqlUpdateByPriKey = "update sysbanner set `Img`=?Img,`Desc`=?Desc,`TimeOutType`=?TimeOutType,`TimeOutDesc`=?TimeOutDesc,`SendType`=?SendType,`SendDesc`=?SendDesc,`TransferType`=?TransferType,`Transfer`=?Transfer,`OperateTime`=?OperateTime,`Operater`=?Operater where `ID`=?ID;";
        //根据主键更新整行数据
        protected const string SqlDeleteByPriKey = "delete from sysbanner  where `ID`=?ID;";
        #endregion
            ...
            var data = new SysbannerDb
                {
                    ...
                    TimeOutDesc = DbChange.ToString(dr["TimeOutDesc"]),
