...
namespace OneCoin.Service.Helper.Http
{
    public class ImgHelper
    {
        ...
        {
            //创建目录
            var dir = Path.GetDirectoryName(fileSaveUrl);
        ...
        public static void CutForCustom(Stream fromFile, string fileSaveUrl, int maxWidth, int maxHeight, int quality)
        {
            ...
            {
                //模版的宽高比例
                var templateRate = (double)maxWidth / maxHeight;
                //原图片的宽高比例
                var initRate = (double)initImage.Width / initImage.Height;

                //原图与模版比例相等，直接缩放
                if (Math.Abs(templateRate - initRate) < 0.000001)
                {
                    //按模版大小生成最终图片
        ...
        public static byte[] ZoomAuto(byte[] imageData, Double targetWidth, Double targetHeight, string watermarkText,string watermarkImage)
        {
            //原始图片（获取原始图片创建对象，并使用流中嵌入的颜色管理信息）
            using (var io = new MemoryStream(imageData))
            {
                var initImage = Image.FromStream(io);
                ...
                if (initImage.Width <= targetWidth && initImage.Height <= targetHeight)
                {
                    ...
                    //保存
                    byte[] buffer;
                    using (var outio = new MemoryStream())
                    {
                        initImage.Save(outio, initImage.RawFormat);

                        buffer = new byte[outio.Length];
                        outio.Position = 0;

                        outio.Read(buffer, 0, (int) outio.Length);
                    }
                            ...
                            if (File.Exists(watermarkImage))
                            {
                                ...
                                using (var wrImage = Image.FromFile(watermarkImage))
                                {
                                    ...
                                    if (newImage.Width >= wrImage.Width && newImage.Height >= wrImage.Height)
                                    {
                                        ...
                                        imgAttributes.SetColorMatrix(wmColorMatrix, ColorMatrixFlag.Default,
                                                                     ColorAdjustType.Bitmap);
                                        gWater.DrawImage(wrImage,new Rectangle(newImage.Width - wrImage.Width,
